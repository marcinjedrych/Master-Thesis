...
context_names = ['D1H','D2H','D3H','D1L','D2L','D3L']
...
prob_h1 = [0.1,0.9] # what is the probability of high for deck1
prob_h2 = [0.50,0.50] # what is the probability of high for deck2
prob_h3 = [0.9,0.1] # what is the probability of high for deck3
...
for choice_id, choice_name in enumerate(choice_names):
    ...
    if choice_name == 'Start':
        ...
        A_behavior[1:,:,choice_id] = np.array([ [pH1, 0 , 0,pL1, 0 , 0], [1-pH1, 1, 1,1-pL1, 1, 1]])
    
    elif choice_name == 'ChD2':
        
        A_behavior[1:,:,choice_id] = np.array([ [0, pH2, 0,0, pL2, 0], [1, 1-pH2, 1,1, 1-pL2, 1]])
    
    elif choice_name == 'ChD3':
        
        A_behavior[1:,:,choice_id] = np.array([ [0, 0, pH3,0, 0, pL3], [1, 1, 1-pH3,1, 1, 1-pL3]])
...
C[0][1] = 0.6
C[0][2] = 0.1
#low reward (0.2) is leuker dan niks, maar high reward nog leuker (0.5)


##D
D = utils.obj_array(num_factors)
D_context = np.array([1/6,1/6,1/6,1/6,1/6,1/6])
...
class deck_environment(object):

  def __init__(self, context = None, p_consist = 0.8):

    self.context_names1 = ['D1H','D1L']
    self.context_names2 = ['D2H','D2L']
    self.context_names3 = ['D3H','D3L']

    if context == None:
        
      self.context1 = self.context_names1[utils.sample(np.array(prob_h1))] # randomly sample
      self.context2 = self.context_names2[utils.sample(np.array(prob_h2))] # randomly sample
      self.context3 = self.context_names3[utils.sample(np.array(prob_h3))] # randomly sample
  ...
  def step(self, action):
    ...
    if action == "Start":
      ...
      if self.context2 == "D2H":
        observed_behavior = self.behavior_obs_names[utils.sample(np.array([0.0, self.p_consist, 1 - self.p_consist]))]
      elif self.context2 == "D2L":
          observed_behavior = self.behavior_obs_names[utils.sample(np.array([0.0, 1 - self.p_consist, self.p_consist]))]
      else:
        observed_behavior = self.behavior_obs_names[utils.sample(np.array([0.0, 1 - self.p_consist, self.p_consist]))]
      ...
      if self.context3 == "D3H":
        observed_behavior = self.behavior_obs_names[utils.sample(np.array([0.0, self.p_consist, 1 - self.p_consist]))]
      elif self.context3 == "D3L":
          observed_behavior = self.behavior_obs_names[utils.sample(np.array([0.0, 1 - self.p_consist, self.p_consist]))]
      else:
        observed_behavior = self.behavior_obs_names[utils.sample(np.array([0.0, 1 - self.p_consist, self.p_consist]))]
...
env = deck_environment(p_consist = p_consist)
T = 200
