...
def runningmodel(pref = 0.1, env_nr = 0):  
    
    N = 5         #amount of participants
...
#------------------------------------------------------------------------------------
def mean_percentage(pref = 0.1, env_nr = 0):
    
    percentage_list1,percentage_list2,percentage_list3 = [],[],[]
    for i in range(5):
        x, n = runningmodel(pref = pref, env_nr = env_nr)
        if len(x) == 0:
...
def heatmap():
    ...
    im = ax[0].imshow(gaussian_filter(explorationdata,sigma =5), cmap = 'Blues')
    im2 = ax[1].imshow(gaussian_filter(exploitationdata,sigma = 5), cmap = 'Reds')
    im3 = ax[2].imshow(gaussian_filter(randomdata,sigma =5), cmap = 'Greens')
    
    ax[0].set_title('Directed Exploration')
    ax[1].set_title('Exploitation')
    ax[2].set_title('Random Exploration')
    
    # Show all ticks and label them with the respective list entries
    for i in range(3):
        ax[i].set_xticks(np.arange(len(prefname)))
        ax[i].set_yticks(np.arange(len(rewardcontextname)))
        ax[i].set_xticklabels(prefname)
        ax[i].set_yticklabels(rewardcontextname)
