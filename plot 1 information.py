...
#Unequal condition
U, SdU = data(pref = 0.5, eq = False) #cte pref

#Equal condition
E, SdE = data(pref = 0.5, eq = True) #cte pref
...
def plot1x2(data = 0):
    ...
    for col in range(cols):
        if col == 0:
            print(row,col,data[0][col])
            plt.subplot(1,2,1)
            plt.bar(xb,data[0][col], color = color, yerr = data[1][col])  
            plt.xlabel('Unequal condition', fontsize = fs)
            plt.ylim([0,1])
            plt.xticks([])
            plt.legend(colors, labels)
        if col == 1:
            ...
            plt.subplot(1,2,2)
            plt.bar(xb,data[0][col], color = color, yerr = data[1][col])
            plt.xlabel('Equal condition', fontsize = fs)
            plt.ylim([0,1])
            plt.xticks([])
